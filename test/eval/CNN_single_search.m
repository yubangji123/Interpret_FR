...
%     fclose(fileID);
    ...
    for p = 1 : nGallery
        ...
        if length(idx) == 1
            gallery_indexs{p} = sightingID2idx(gallery.sightingID(idx));
        else
    ...
    for p = 1 : nProbe
        ...
        if length(idx) == 1
            probe_indexs{p} = sightingID2idx(probe.sightingID(idx));
        else
    ...
    acc = (sum(esLabels==probe_subject)) / nProbe ;
